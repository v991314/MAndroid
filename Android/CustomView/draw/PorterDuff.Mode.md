...
PorterDuff.Mode类主要用于图形合成时的图像过渡模式计算，其概念来自于1984年在ACM SIGGRAPH计算机图形学出版物上发表了“Compositing digital images（合成数字图像）”的Tomas Porter和Tom Duff，合成图像的概念极大地推动了图形图像学的发展，PorterDuffXfermode类名就来源于这俩人的名字组合PorterDuff。

> 注释说明

* alpha_{out} 输出通过计算之后的Alpha
* C_{out} 输出通过计算之后的颜色
* alpha_{dst} 目标图像的Alpha
* C_{dst} 目标图片的颜色
* alpha_{src} 源图的Alpha
* C_{src} 源图的颜色
