...
/**
 * Java层Binder系统是Native层Binder系统的一个镜像，终归需要借助native层的Binder系统来开展工作
 *
 * register_android_os_Binder专门负责搭建JavaBinder和nativeBinder关系
 * 主要完成了三个初始化工作：Binder类的初始化、BinderInternal类的初始化、BinderProxy类的初始化
 *
...
public interface IBinder {
    ...
    /**
     * 当客户端利用Binder调用一个发起跨进程调用时，客户端一般会阻塞直到结果返回。
     * 但在设置该标志时，客户端只需将请求发送到Binder驱动即可而不用等待服务器结果，这就是所谓的非阻塞方式
     *
     * 对于使用该标志的函数来说，客户端一般会向服务端注册一个回调（同样是跨进程的Binder回调），一旦服务器处理了该请求，通过回调返回请求结果
     *
