...
//BpBinder是客户端用来和Server交互的代理类，p即Proxy代理
//BBinder则是与BpBinder相对的一端，它是与proxy交互的目的端

//BpBinder和BBinder都是从IBinder派生出来
//如果说BpBinder代表客户端，BBinder代表服务器端。BpBinder与BBinder是一一对应的

//Binder系统通过handle来标识对应BBinder

//BpBinder、BBinder这两个类没有任何地方操作ProcessState打开的那个/dev/binder设备
//BpBinder只是道具
// ---------------------------------------------------------------------------
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    ...
    if (mAlive) {
        //BpBinder果然是道具，它把transact工作交给了IPCTreadState,它是进程中真正干活的伙计
