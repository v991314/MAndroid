                                  ...
                                  Parcel* reply, uint32_t flags)
{
    ...
    if (err == NO_ERROR) {
        LOG_ONEWAY(">>>> SEND from pid %d uid %d %s", getpid(), getuid(),
            ...
            而binder设备向应用程序回复消息的消息码一BR_开头。消息码的定义在binder_module.h中
    ...
    : mProcess(ProcessState::self()), mMyThreadId(androidGetTid())
{
    ...
    //每个线程都有一个IPCTreadState，每个IPCThreadState都有一个mIn,mOut
    //其中mIn用来接受来自Binder设备的数据
    //mOut则是用来存储发往Binder设备的数据
}
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    //接受数据缓冲区信息的填充。如果以后收到数据，就直接填在mIn中
...
{
    //binder_transaction是和binder设备通信的数据结构
    binder_transaction_data tr;
