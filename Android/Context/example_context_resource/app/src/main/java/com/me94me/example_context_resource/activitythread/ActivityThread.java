...
public final class ActivityThread extends ClientTransactionHandler {
    ...
    private void attach(boolean system, long startSeq) {
        ...
        //正常App
        if (!system) {
            //给ViewRootImpl添加handler确保第一次绘制的时候Jit能用
            ...
            BinderInternal.addGcWatcher(new Runnable() {
                @Override public void run() {
                    ...
                    //每启动一个activity就会检测内存限制是否超标，超过了就释放相应的activity同时设为false
    ...
    public static void main(String[] args) {
        ...
        // CloseGuard默认为true，可能非常垃圾。
        // 我们在这里禁用它，但是稍后在调试版本上选择性地启用它（通过StrictMode），但是使用DropBox而不是日志。
        ...
        TrustedCertificateStore.setDefaultUser(configDir);

        //改变进程argv[0]的参数
        //这对于在'ps'命令之类的内容中显示更多描述性信息非常有用。
        Process.setArgV0("<pre-initialized>");

        //准备主线程循环
        Looper.prepareMainLooper();

        // 如果在命令行中提供，请查找{@link #PROC_START_SEQ_IDENT}的值。
        // 它的格式为“seq = 114”
        long startSeq = 0;
        //Process.setArgV0("<pre-initialized>");设置了args[0]的值
        if (args != null) {
            for (int i = args.length - 1; i >= 0; --i) {
                if (args[i] != null && args[i].startsWith(PROC_START_SEQ_IDENT)) {// "seq="
